-- Remove família de peça no produto (precificação fica só na tag).

alter table public.products
  drop constraint if exists products_parts_family_check;

alter table public.products
  drop column if exists parts_family;

-- Troca do tipo de retorno exige drop (CREATE OR REPLACE não altera OUT/returns table).

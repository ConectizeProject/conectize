...
import { runMeliTokenRefreshForAllConnections } from '@/lib/integrations/mercado-livre/scheduled-token-refresh'
...
async function verifyCronSecret(request: Request): Promise<boolean> {
  ...
  if (request.method === 'POST') {
    try {
      ...
      if (
        typeof parsed.cronSecret === 'string' &&
        parsed.cronSecret.trim() === secret
      )
        return true
    } catch {
      return false
    ...
    return NextResponse.json(
      {
        ok: false,
        error:
          'SUPABASE_SERVICE_ROLE_KEY necessário para renovar tokens em background',
      },
      { status: 503 },
    )
  }
...
export async function GET(request: Request) {
  ...
  try {
    const summary = await runMeliTokenRefreshForAllConnections()
    const hasFailures = summary.failed.length > 0
    if (hasFailures) {
      return NextResponse.json(
        { ok: false, ...summary, error: 'meli_token_refresh_partial_failure' },
        { status: 500 },
      )
    }

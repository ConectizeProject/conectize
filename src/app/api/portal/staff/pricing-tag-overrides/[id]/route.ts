...
export async function PATCH (
  ...
  if (error) {
    console.error('[staff/pricing-tag-overrides PATCH]', error)
...
export async function DELETE (
  ...
  const { error } = await auth.supabase.from('pricing_tag_retailer_overrides').delete().eq('id', id)
  if (error) {
    console.error('[staff/pricing-tag-overrides DELETE]', error)
    return NextResponse.json({ ok: false, error: 'db_error' }, { status: 500 })
  }

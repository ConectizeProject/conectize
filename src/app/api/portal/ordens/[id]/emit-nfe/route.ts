...
import { emitNfeForServiceOrder, getServiceOrderNfeState } from '@/lib/fiscal/emit-nfce'
...
export async function GET (
  ...
  const fiscalDocument = await getServiceOrderNfeState(auth, orderId)
...
export async function POST (
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: auth.error === 'forbidden'
            ? 'Seu usuário não tem permissão para emitir NF-e.'
    ...
    const result = await emitNfeForServiceOrder(auth, orderId)
    if (result.ok === false) {
    ...
    console.error('[ordens/emit-nfe] unhandled', err)
    const message = err instanceof Error && err.message.trim()
      ? err.message
      : 'Erro inesperado ao emitir a NF-e.'
    return NextResponse.json(

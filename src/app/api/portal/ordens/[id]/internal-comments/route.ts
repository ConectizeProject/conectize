...
export async function GET (
  ...
  if (error) {
    console.error('[internal-comments GET]', error)
...
export async function POST (
  ...
  const { error } = await auth.supabase.from('service_order_internal_comments').insert({
  ...
  if (error) {
    console.error('[internal-comments POST]', error)
    return NextResponse.json({ ok: false, error: 'db_error' }, { status: 500 })
  }

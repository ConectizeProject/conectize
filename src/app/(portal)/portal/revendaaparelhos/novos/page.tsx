...
export default async function RevendaNovosPage ({
  searchParams,
}: {
  ...
  return (
    <Suspense fallback={<div className="p-4 text-sm text-muted-foreground">Carregando…</div>}>
      <RevendaNovosInner searchParams={searchParams} />
    </Suspense>
  )
}

async function RevendaNovosInner ({
  searchParams,
}: {
  searchParams: SearchParams
}) {
  ...
  const [devicesRaw, stats, distinctDeviceNames] = await Promise.all([
    ...
    fetchResaleDistinctDeviceNames(supabase, 'lacrado'),
  ])

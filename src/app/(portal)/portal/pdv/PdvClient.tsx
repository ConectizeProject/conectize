...
import {
	readTopProductsCache,
	writeTopProductsCache,
} from './pdv-top-products-cache'
import type {
	CartItem,
	CatalogProduct,
	OrderSummary,
...
const SalesOrderAfterSaleDialog = dynamic(
	() =>
		import('@/app/(portal)/portal/vendas/SalesOrderAfterSaleActions').then(
			(m) => ({
...
async function openCashClosePrintLazy(
	...args: Parameters<
		typeof import('@/lib/pdv/open-cash-close-print').openCashClosePrint
	>
) {
...
async function openSalesOrderCupomPrintLazy(orderId: string) {
	const { openSalesOrderCupomPrint } = await import(
		'@/app/(portal)/portal/vendas/SalesOrderCupomPrint'
	)
	return openSalesOrderCupomPrint(orderId)
}

async function openNfceDanfePrintLazy(documentId: string) {
	const { openNfceDanfePrint } = await import(
		'@/app/(portal)/portal/vendas/SalesOrderCupomPrint'
	)
...
function pdvNfceMenuLabel(order: OrderSummary) {
	...
	if (order.nfce_status === 'rejected' || order.nfce_status === 'denied')
		return 'Abrir NFC-e'
	return 'Gerar NFC-e'
}
		...
		row.id === orderId
			? {
					...
					nfce_status: (nfce.status ||
						row.nfce_status) as OrderSummary['nfce_status'],
...
export function PdvClient({
	...
	const [itemDiscountMode, setItemDiscountMode] = useState<'fixed' | 'percent'>(
		'fixed',
	)
	const [editingCartLineId, setEditingCartLineId] = useState<string | null>(
		null,
	)
	...
	const customerNameDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	)
	...
	const [afterSaleOrderNumber, setAfterSaleOrderNumber] = useState<
		number | string | null
	>(null)
	...
	const [closeCountedByMethod, setCloseCountedByMethod] = useState<
		Partial<Record<PaymentMethodType, string>>
	>({})
	...
	const discountTotalCents = useMemo(() => {
		if (orderDiscountMode === 'percent') {
			const pct = Math.min(
				100,
				Math.max(0, Number(discountTotalMasked.replace(',', '.')) || 0),
			)
			return Math.round((subtotalCents * pct) / 100)
		}
		return Math.min(
			subtotalCents,
			moneyToCentsFromMasked(discountTotalMasked) || 0,
		)
	...
	const totalCents =
		Math.max(0, subtotalCents - discountTotalCents) + surchargeCents
	const customerType = customerTypeFromDocument(customerDocument)
	const hasCartItems = cart.length > 0
	const paidCents = useMemo(
		() =>
			payments.reduce(
				(acc, p) => acc + (moneyToCentsFromMasked(p.amountMasked) || 0),
				0,
			),
		[payments],
	)
	...
	const hasCashPayment = payments.some(
		(p) => p.payment_method_type === 'dinheiro',
	)
	...
	const insertDiscountCents = useMemo(() => {
		if (itemDiscountMode === 'percent') {
			const pct = Math.min(
				100,
				Math.max(0, Number(itemDiscountMasked.replace(',', '.')) || 0),
			)
	...
	const insertSubtotalCents = Math.max(
		0,
		insertQty * insertUnitCents - insertDiscountCents,
	)
	...
	useEffect(() => {
		...
		const run = () => {
			void loadTopProducts({ force: true })
		}
	...
	const loadMethods = useCallback(async () => {
		try {
			...
			if (data?.ok && Array.isArray(data.paymentMethods)) {
				...
				if (organizationId) {
					try {
						await writeOfflinePaymentMethods(
							organizationId,
							data.paymentMethods,
						)
					} catch (err) {
						console.warn(
							'[pdv-offline] falha ao gravar formas de pagamento no IndexedDB',
							err,
						)
					}
				}
	...
	const prefetchCatalogSnapshot = useCallback(
			...
			try {
				...
				if (data?.ok && Array.isArray(data.products)) {
					setSearchResults(
						data.products
							.slice(0, 10)
							.map((row: Record<string, unknown>) => mapCatalogProduct(row)),
					)
	...
	const lookupProductByBarcode = useCallback(
		async (code: string) => {
			...
			try {
				const res = await portalFetch(
					`/api/portal/pdv/catalog?barcode=${encodeURIComponent(barcode)}`,
				)
				const data = await res?.json().catch(() => null)
				if (
					!data?.ok ||
				...
				return mapCatalogProduct(
					(exact || data.products[0]) as Record<string, unknown>,
				)
			} catch {
				return null
			}
	...
	useEffect(() => {
		setPayments((prev) => {
			...
			if (
				receivedCents > 0 &&
				prev.some((line) => line.payment_method_type === 'dinheiro')
			) {
	...
	const persistOrderDraft = useCallback(
		async (
			...
			if (!orderId) {
				...
				if (!silent) {
					toast({
						title: data?.error || 'Erro ao salvar pedido',
						variant: 'destructive',
					})
				}
				return { ok: false }
			...
			if (!silent) {
				toast({
					title: data?.error || 'Erro ao atualizar pedido',
					variant: 'destructive',
				})
			}
			return { ok: false }
		...
		void persistOrderDraft(payload, orderId, {
			...
			if (result.ok) {
				if (options?.successToast !== false) {
					const orderLabel =
						result.orderNumber != null ? `#${result.orderNumber}` : ''
					toast({
						title: orderLabel ? `Pedido ${orderLabel} salvo` : 'Pedido salvo',
						description:
							'Venda anterior ficou em andamento. Você pode retomá-la na lista.',
					})
				}
				return
			}
	...
	async function handleSearchSubmit() {
		...
		try {
			...
			if (catalogCache.length > 0) {
				...
				const res = await portalFetch(
					`/api/portal/pdv/catalog?q=${encodeURIComponent(code)}`,
				)
				const data = await res?.json().catch(() => null)
	...
	function customerDisplayName(row: {
		...
		if (row.is_company) {
			return String(
				row.company_name || row.trade_name || row.full_name || 'Empresa',
			).trim()
		}
	...
	function applyCustomerFromCadastro(row: CustomerMatch['raw']) {
		...
		const digits = String(isCompany ? row.cnpj || '' : row.cpf || '').replace(
			/\D/g,
			'',
		)
		...
		setCustomerName(
			clampFiscalCustomerName(name || (isCompany ? 'Empresa' : 'Cliente')),
		)
	...
	async function lookupCustomerByDocument(digits: string) {
		...
		try {
			...
			const exact = data.customers.find(
				(row: { cpf?: string | null; cnpj?: string | null }) => {
	...
	async function searchCustomersByName(name: string) {
		...
		try {
			const res = await portalFetch(
				`/api/portal/customers/search?q=${encodeURIComponent(q)}`,
				{
					skipAuthRedirect: true,
				},
			)
			...
			const matches: CustomerMatch[] = data.customers
				.slice(0, 10)
				.map((row: CustomerMatch['raw']) => {
					const isCompany = Boolean(row.is_company)
					const digits = String(
						isCompany ? row.cnpj || '' : row.cpf || '',
					).replace(/\D/g, '')
					return {
						id: String(row.id),
	...
	async function ensureCustomerInCadastro() {
		...
		const isDefaultConsumer =
			!digits && (!name || name.toLowerCase() === 'consumidor final')
	...
	function updateCartItem(lineId: string, patch: Partial<CartItem>) {
		setCart((prev) =>
			prev.map((item) =>
				item.lineId === lineId ? { ...item, ...patch } : item,
			),
		)
	}
	...
	async function loadOrderIntoCart(orderId: string) {
		...
		setCustomerName(
			clampFiscalCustomerName(data.order.customer_name || 'Consumidor Final'),
		)
		setCustomerDocument(
			formatCpfCnpj(String(data.order.customer_document || '')),
		)
		setCustomerId(null)
		setCustomerMatches([])
		setShowCustomerDropdown(false)
		setOrderDiscountMode('fixed')
		setDiscountTotalMasked(
			maskedFromCents(data.order.discount_total_cents || 0),
		)
		setSurchargeMasked(
		...
		const payLines: PaymentLine[] = (data.payments ?? []).map(
			(p: {
	...
	async function openCash() {
		...
		toast({
			title: data?.error || 'Erro ao abrir caixa',
			variant: 'destructive',
		})
	}
	...
	async function submitMovement(type: 'sangria' | 'suprimento') {
		...
		if (data?.ok) {
			...
			toast({
				title:
					type === 'sangria' ? 'Sangria registrada' : 'Suprimento registrado',
			})
			return
		}
		toast({
			title: data?.error || 'Erro ao registrar movimento',
			variant: 'destructive',
		})
	}
	...
	const loadCloseSummary = useCallback(async () => {
		...
		if (data?.ok && data.summary) {
			...
			if (data?.error === 'cash_not_open') {
				...
				toast({
					title: 'Erro ao carregar resumo do caixa',
					variant: 'destructive',
				})
			}
		}
	...
	function getCloseMethodDifferenceCents(type: PaymentMethodType) {
		...
		const counted =
			moneyToCentsFromMasked(closeCountedByMethod[type] || '') ?? 0
	...
	async function closeCash() {
		...
		const nonCashMethods = closeSummary.methods_used.filter(
			(type) => type !== 'dinheiro',
		)
		...
		setBusy(false)
		if (data?.ok) {
			const closedSessionId = String(
				data.session?.id || closeSummary.session_id || '',
			)
		...
		toast({
			title: data?.error || 'Erro ao fechar caixa',
			variant: 'destructive',
		})
	}
	...
	function printCloseReport() {
		...
		void openCashClosePrintLazy({
			...
			countedCashCents,
			countedByMethod:
				Object.keys(countedByMethod).length > 0 ? countedByMethod : null,
		})
	}
	...
	async function cancelCurrentOrder() {
		...
		const status =
			sessionOrders.find((o) => o.id === currentOrderId)?.status ??
			'in_progress'
	...
	async function cancelOrderById(
		orderId: string,
		statusHint?: OrderSummary['status'] | null,
	) {
		const status =
			statusHint ||
		...
		setBusy(true)
		const res = await portalFetch(
			`/api/portal/sales-orders/${orderId}/cancel`,
			{
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
	...
	async function emitNfceForOrder(order: OrderSummary) {
		...
		try {
			...
			if (
				...
				(fiscalDocument.status === 'rejected' ||
					...
					(fiscalDocument.status === 'pending' &&
						isProductFiscalCorrectionError(fiscalDocument.sefaz_status_code)))
			) {
				setSessionOrders((prev) =>
					patchSessionOrderNfce(prev, order.id, fiscalDocument),
				)
				router.push(
					nfceEditorHref(fiscalDocument.id, {
						corrigir: isProductFiscalCorrectionError(
							fiscalDocument.sefaz_status_code,
						),
					}),
				)
				return
			...
			if (stateData?.danfe_url && fiscalDocument?.id) {
				setSessionOrders((prev) =>
					patchSessionOrderNfce(prev, order.id, {
			...
			const endpoint =
				fiscalDocument?.id && fiscalDocument.status !== 'authorized'
					? `/api/portal/fiscal/documents/${encodeURIComponent(fiscalDocument.id)}/retry`
					: `/api/portal/sales-orders/${encodeURIComponent(order.id)}/emit-nfce`

			const res = await portalFetch(endpoint, { method: 'POST' })
			const data = await res?.json().catch(() => null)
			const nextDocument = (data?.fiscal_document ??
				null) as PdvNfceDocument | null
			if (!data?.ok) {
				if (data?.needs_correction && nextDocument?.id) {
					setSessionOrders((prev) =>
						patchSessionOrderNfce(prev, order.id, nextDocument),
					)
					toast({
						title: 'Complete NCM e CEST',
						description:
							data.message ||
							'Preencha os dados fiscais dos produtos para emitir a NFC-e.',
					})
			...
			toast({
				title: 'NFC-e não autorizada',
				description:
					nextDocument?.sefaz_status_message ||
	...
	function finalizeOrder() {
		...
		if (paidCents > totalCents) {
			toast({
				...
				description:
					'A soma das formas de pagamento não pode passar do total da venda. Use “Recebido em dinheiro” para o troco.',
				variant: 'destructive',
			})
			return
		}
		...
		if (hasPaymentWithoutMethod) {
			toast({
				...
				description:
					'É necessário vincular a carteira no financeiro para lançar a movimentação.',
				variant: 'destructive',
			})
			return
		}
			...
			try {
				...
				if (!customerSync.ok) {
					toast({
						...
						description: String(
							customerSync.error || 'A venda seguirá mesmo assim.',
						),
						variant: 'destructive',
					})
				...
				if (!finalizeData?.ok) {
					const errorTitle =
						finalizeData?.error === 'finance_sync_failed'
				...
				const finalOrderId = String(
					finalizeData.order_id || finalizeData.order?.id || '',
				)
	...
	useEffect(() => {
		function onKeyDown(event: KeyboardEvent) {
			...
			if (key === 'F5') {
				...
				setActiveTab('pagamento')
				requestAnimationFrame(() =>
					document.getElementById('pdv-customer-document')?.focus(),
				)
				return
			}
			if (key === 'F6') {
			...
			if (key === 'F7') {
				...
				if (
					payments.some((p) => p.payment_method_type === 'dinheiro') ||
					hasCashPayment
				) {
		...
		<div className="flex h-full min-h-0 w-full flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm">
			<header className="flex shrink-0 items-center justify-between rounded-t-xl border-b bg-primary px-4 py-2 text-primary-foreground">
				<div className="flex items-center gap-4">
					...
					<span className="hidden text-[11px] opacity-75 lg:inline">
						F2 busca · F3 qtd · F4 pag. · F5 cliente · F6 finalizar · F7 exato ·
						F9 nova
					</span>
				</div>
					...
					{cashOpen ? (
						<DropdownMenu>
							...
							<DropdownMenuContent align="end">
								...
								<DropdownMenuItem onClick={() => setSangriaOpen(true)}>
									Sangria
								</DropdownMenuItem>
								<DropdownMenuItem onClick={() => setSuprimentoOpen(true)}>
									Suprimento
								</DropdownMenuItem>
								<DropdownMenuItem onClick={() => setCloseCashOpen(true)}>
									Fechar caixa
								</DropdownMenuItem>
							</DropdownMenuContent>
						</DropdownMenu>
					...
					<div className="absolute inset-0 z-20 flex items-center justify-center bg-background/80 backdrop-blur-[1px]">
						<div className="mx-4 flex max-w-sm flex-col items-center gap-3 rounded-xl border bg-card px-6 py-8 text-center shadow-sm">
							...
							<p className="text-sm text-muted-foreground">
								Abra o caixa para iniciar vendas, lançar pagamentos e registrar
								movimentos.
							</p>
					...
					<div className="grid min-h-0 flex-1 grid-cols-2">
						...
						<div className="flex flex-col overflow-hidden border-r">
							...
							>
								...
								<TabsContent
									value="produto"
									className="flex min-h-0 flex-1 flex-col overflow-hidden p-4 data-[state=inactive]:hidden"
								>
										...
										{loadingTopProducts && topProducts.length === 0 ? (
											...
											<div className="mt-1 grid grid-cols-5 gap-1">
												{topProducts.slice(0, 5).map((product, index) => (
													...
													>
														{product.image_url &&
														isSafeProductListImageUrl(product.image_url) ? (
															<ProductThumbImage
														...
														<div className="absolute inset-x-0 bottom-0 bg-black/65 px-1 py-0.5">
															<div className="truncate text-[9px] font-medium leading-tight text-white">
																{product.name}
															</div>
									...
									<div ref={searchRef} className="relative mb-3 shrink-0">
										<div className={inputGroupShell}>
											...
											<input
												...
												onFocus={() =>
													query.trim() && setShowSearchDropdown(true)
												}
												onKeyDown={(e) => {
													if (e.key === 'Enter') {
														e.preventDefault()
									...
									<div className="grid shrink-0 grid-cols-2 items-end gap-2 md:flex md:flex-wrap">
										...
										<div className={insertFormFieldClass}>
											...
											<div className="mt-1 w-full">
												<DiscountField
													...
													onModeToggle={() => {
														setItemDiscountMode((m) =>
															m === 'fixed' ? 'percent' : 'fixed',
														)
														setItemDiscountMasked('')
													}}
													disabled={!selectedProduct}
										...
										<div className={insertFormFieldClass}>
											...
											<Input
												readOnly
												value={maskedFromCents(insertSubtotalCents)}
												className="mt-1 h-9 w-full min-w-0 bg-muted px-2 text-xs"
												tabIndex={-1}
											/>
										</div>
										{editingCartLineId ? (
												...
												<div className="relative mt-1">
													<Input
														...
														onChange={(e) => {
															setCustomerId(null)
															setCustomerName(
																clampFiscalCustomerName(e.target.value),
															)
														}}
														onFocus={() => {
													...
													<div className="absolute left-0 right-0 top-full z-50 mt-1 max-h-56 overflow-y-auto rounded-md border bg-popover text-popover-foreground shadow-md">
														{customerMatches.map((match) => (
															<button
																...
																className="block w-full px-3 py-2 text-left text-sm hover:bg-accent"
																onClick={() =>
																	applyCustomerFromCadastro(match.raw)
																}
															>
									...
									<table className="w-full table-fixed text-sm">
										<thead className="sticky top-0 z-10 border-b bg-muted/70">
											<tr className="text-left text-xs font-medium text-muted-foreground">
												...
												<th className="w-[5.5rem] px-2 py-2.5 text-right">
													Preço
												</th>
												<th className="w-[6rem] px-2 py-2.5 text-right">
													Sub Total
												</th>
												<th className="w-[4.5rem] px-2 py-2.5 text-center">
													Ações
												</th>
											</tr>
										</thead>
													...
													<td className="px-1 py-2 text-center">
														<div className="flex items-center justify-center gap-0">
															<Button
																...
																className="h-8 w-8"
																onClick={() =>
																	void beginEditCartItem(item.lineId)
																}
															...
															<Button
																...
																onClick={() => {
																	if (editingCartLineId === item.lineId)
																		cancelCartLineEdit()
									...
									<>
										...
										{discountTotalCents > 0 ? (
											<div className="flex items-center justify-between text-sm font-semibold">
												<span>Desconto</span>
												<span className="tabular-nums">
													− {maskedFromCents(discountTotalCents)}
												</span>
											</div>
										) : null}
										{surchargeCents > 0 ? (
											<div className="flex items-center justify-between text-sm font-semibold">
												...
												<span className="tabular-nums">
													+ {maskedFromCents(surchargeCents)}
												</span>
											</div>
										) : null}
									</>
										...
										>
											...
											<DropdownMenu>
												...
												<DropdownMenuContent align="end">
													<DropdownMenuItem
														onClick={() =>
															void openSalesOrderCupomPrintLazy(order.id)
														}
													>
													...
													{order.status === 'paid' ? (
														<DropdownMenuItem
															...
															onClick={() => {
																if (
																	order.nfce_status === 'authorized' &&
																	order.nfce_document_id
																) {
																	void openNfceDanfePrintLazy(
																		order.nfce_document_id,
																	)
																	return
																}
																if (
																	...
																	router.push(
																		nfceEditorHref(order.nfce_document_id),
																	)
																	return
																}
													...
													{isActive || order.status === 'paid' ? (
														<DropdownMenuItem
															...
															disabled={busy}
															onClick={() =>
																void cancelOrderById(order.id, order.status)
															}
														>
															{order.status === 'paid'
																? 'Estornar venda'
			...
			<Dialog open={sangriaOpen} onOpenChange={setSangriaOpen}>
				<DialogContent aria-describedby={undefined}>
					...
					<div className="space-y-3">
						<div>
							...
							<Input
								...
								onChange={(e) =>
									setMovementAmount(formatMoneyInput(e.target.value))
								}
								placeholder="0,00"
							/>
						</div>
					...
					<DialogFooter>
						<Button
							onClick={() => void submitMovement('sangria')}
							disabled={busy}
						>
							Confirmar
						</Button>
			...
			<Dialog open={suprimentoOpen} onOpenChange={setSuprimentoOpen}>
				<DialogContent aria-describedby={undefined}>
					...
					<DialogFooter>
						<Button
							onClick={() => void submitMovement('suprimento')}
							disabled={busy}
						>
							Confirmar
						</Button>
			...
			<Dialog open={closeCashOpen} onOpenChange={setCloseCashOpen}>
				...
				>
						...
						<div className="space-y-4 py-1">
							<div className="space-y-2 rounded-lg border bg-muted/30 p-3 text-sm">
								...
								<div className="flex justify-between gap-2">
									...
									<strong>
										{maskedFromCents(closeSummary.opening_amount_cents)}
									</strong>
								</div>
								<div className="flex justify-between gap-2">
									<span className="text-muted-foreground">Sangrias</span>
									<strong className="text-destructive">
										− {maskedFromCents(closeSummary.sangrias_cents)}
									</strong>
								</div>
								{closeSummary.suprimentos_cents > 0 ? (
									<div className="flex justify-between gap-2">
										...
										<strong>
											+ {maskedFromCents(closeSummary.suprimentos_cents)}
										</strong>
									</div>
								) : null}
								<div className="border-t pt-2">
									<p className="mb-1.5 text-xs text-muted-foreground">
										Recebido no sistema por forma de pagamento
									</p>
										...
										<ul className="space-y-1">
											{closeSummary.methods_used.map((type) => (
												<li key={type} className="flex justify-between gap-2">
													...
													<strong>
														{maskedFromCents(closeSummary.by_method[type])}
													</strong>
												</li>
											))}
										</ul>
								...
								<div className="flex justify-between gap-2 border-t pt-2">
									...
									<strong>
										{maskedFromCents(closeSummary.expected_cash_cents)}
									</strong>
								</div>
								<p className="text-xs text-muted-foreground">
									{closeSummary.paid_orders_count} venda(s) finalizada(s) nesta
									sessão
								</p>
							</div>
							...
							<div className="space-y-3">
								...
								<div>
									...
									<Input
										...
										onChange={(e) =>
											setCloseCountedCash(formatMoneyInput(e.target.value))
										}
										placeholder="0,00"
										className="mt-1"
									/>
									{closeCashDifferenceCents != null ? (
										...
										>
											Diferença no caixa:{' '}
											{closeCashDifferenceCents >= 0 ? '+' : '−'}{' '}
											{maskedFromCents(Math.abs(closeCashDifferenceCents))}
										</p>
									) : null}
								</div>
								...
								{closeSummary.methods_used
									...
									.map((type) => {
										...
										return (
											<div key={type}>
												<Label>
													Valor conferido em {PAYMENT_METHOD_LABELS[type]}
												</Label>
												<p className="text-xs text-muted-foreground">
													Sistema:{' '}
													{maskedFromCents(closeSummary.by_method[type])}
					...
					<DialogFooter className="flex-col gap-2 sm:flex-row sm:justify-end">
						<Button
							variant="outline"
							onClick={() => setCloseCashOpen(false)}
							disabled={busy}
						>
							Cancelar
						</Button>

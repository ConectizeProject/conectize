...
function buildPrintHtml(data: OrdemPrintData, company?: CompanyPrintData | null, origin?: string): string {
  const base = origin || (typeof window !== 'undefined' ? window.location.origin : '')
  const logoFullUrl = company?.logoUrl
    ? (company.logoUrl.startsWith('http') ? company.logoUrl : `${base}${company.logoUrl.startsWith('/') ? company.logoUrl : '/' + company.logoUrl}`)
    : ''
...
<body>
  ...
  ` : ''}
  <h1>Ordem de Serviço #${data.displayNumber ?? '-'} — ${company?.name || 'Conectize'}</h1>
  ...
  <div class="footer">
    Impresso em ${new Date().toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })} — ${company?.name || 'Conectize Assistência Técnica'}
  </div>
...
export function OrdemPrintButton({ data, company }: { data: OrdemPrintData; company?: CompanyPrintData | null }) {
  function handlePrint() {
    const origin = typeof window !== 'undefined' ? window.location.origin : ''
    const html = buildPrintHtml(data, company, origin)

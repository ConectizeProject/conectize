...
function parseOrderPaymentMethods(
	order: any,
): Array<{
...
function parsePaymentMethodsJson(
	raw: unknown,
): Array<{
	payment_method_id: string;
	installments?: number;
	...
	try {
		...
		return parsed
			.filter(
				(item: unknown) =>
					item && typeof item === "object" && (item as any).payment_method_id,
			)
			.map((item: any) => {
				...
				return {
					...
					installments:
						item.installments != null
							? Math.max(1, Math.min(24, Number(item.installments) || 1))
							: undefined,
					value_cents:
						item.value_cents != null
							? Math.max(0, Number(item.value_cents) || 0)
							: null,
				};
			})
			.filter(Boolean) as Array<{
...
function parseServicesJson(raw: unknown): {
	...
	try {
		...
		const normalized = items
			...
			.map((item: unknown) => {
				...
				const kind: "service" | "product" =
					i.kind === "product" ? "product" : "service";
		...
		const totalValueCents = normalized.reduce(
...
export default async function OrdemDetalhePage({
	...
	const [
		...
		supabase
			...
			.select(
				"name, cnpj, address, complement, zip_code, city, state, phone, email, logo_url",
			)
			.eq("id", 1)
			.maybeSingle(),
	...
	const deviceString = deviceModel
		? [
				brandName,
				deviceTypeName,
				(deviceModel as any)?.model as string | undefined,
			]
				.filter(Boolean)
				.join(" • ") || ""
						...
						<CardContent className="space-y-6">
							...
							<div className="space-y-2">
								...
								<Textarea
									...
									defaultValue={order.customer_description || ""}
							...
							<div className="space-y-2">
								...
								<Textarea
									id="receivingNotes"
									name="receivingNotes"
									defaultValue={order.receiving_notes || ""}

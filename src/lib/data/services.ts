...
export function getAllBrandPaths (serviceSlug: string) {
  const service = getServiceBySlug(serviceSlug)
  if (!service) return []
  
  return service.brands.map(brand => ({
    servico: serviceSlug,
    marca: brand
  }))
}

export function getAllDeviceTypePaths (serviceSlug: string, brandSlug: string) {
  const service = getServiceBySlug(serviceSlug)
  const brand = getBrandBySlug(brandSlug)
  ...
  return Object.values(brand.deviceTypes)
    ...
    .map(deviceType => ({
      servico: serviceSlug,
      marca: brandSlug,
      tipo: deviceType.slug
    }))
}

export function getAllModelPaths (serviceSlug: string, brandSlug: string, deviceTypeSlug: string) {
  const service = getServiceBySlug(serviceSlug)
  const deviceType = getDeviceTypeBySlug(brandSlug, deviceTypeSlug)
  ...
  const excludedTypes = service.excludedDeviceTypes?.[brandSlug] || []
  if (excludedTypes.includes(deviceTypeSlug)) return []
  
  return deviceType.models.map(model => ({
    servico: serviceSlug,
    marca: brandSlug,
    tipo: deviceTypeSlug,
    modelo: model

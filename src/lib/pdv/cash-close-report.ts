...
function row(
	...
	const labelHtml = opts?.strong
		? `<strong>${escapeHtml(label)}</strong>`
		: escapeHtml(label)
	const valueHtml = opts?.strong
...
export function buildCashCloseReportHtml(
	...
	const closedAt = meta.closedAt
		? formatDateTimeBr(meta.closedAt)
		: formatDateTimeBr(new Date().toISOString())
	const sellerName = escapeHtml(meta.sellerName || '—')

	const methodRows =
		summary.methods_used.length > 0
			? summary.methods_used
					.map((type) =>
						row(
							PAYMENT_METHOD_LABELS[type],
							formatCentsBr(summary.by_method[type]),
						),
					)
					.join('')
	...
	const conferenceMethodRows = PAYMENT_METHOD_TYPES.filter(
		(type) => type !== 'dinheiro' && countedByMethod[type] != null,
	)
		.map((type) => {

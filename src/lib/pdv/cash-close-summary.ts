...
export const PAYMENT_METHOD_TYPES = ['dinheiro', 'pix', 'credito', 'debito', 'outro'] as const
...
export async function buildCashCloseSummary (
...
): Promise<{ ok: true, summary: CashCloseSummary } | { ok: false, error: 'db_error' }> {
  ...
  if (orderIds.length > 0) {
    ...
    for (const row of payments ?? []) {
      ...
      const type = String(row.payment_method_type || 'outro') as PaymentMethodType
  ...
  const expectedCashCents = opening + cashFromOrders - totalChange - sangriasCents + suprimentosCents
...
export function parseCountedByMethod (raw: unknown): Partial<Record<PaymentMethodType, number>> {
